...
This script combines email generation, submission for approval, and monitoring
for CFO approval responses - all in one automated process.

CFO Email: arunsukumar03@gmail.com
Vendor Email: dhivyasreenidhidurai@gmail.com

Usage:
    python automation.py                    # Run with default 1-second interval
    python automation.py --interval 5       # Run with 5-second interval
"""
...
def monitor_approvals(settings: Settings, check_interval: int = 1):
    ...
    console.print(Panel.fit(
        ...
        f"Checking every {check_interval} second(s)\n"
        f"CFO: {CFO_EMAIL}\n"
...
def run_automation(check_interval: int = 1):
    """Run the complete automated workflow."""
    
    console.print(Panel.fit(
        "[bold magenta]🚀 AUTOMATED EMAIL WORKFLOW[/bold magenta]\n\n"
        f"[cyan]CFO (Approver):[/cyan] {CFO_EMAIL}\n"
        f"[cyan]Vendor (Final Recipient):[/cyan] {VENDOR_EMAIL}\n"
        f"[cyan]Check Interval:[/cyan] {check_interval} second(s)\n\n"
        "[dim]This automation will:[/dim]\n"
        "1. Generate 10 test emails (5 discrepancy + 5 regular)\n"
        "2. Send all emails to CFO for approval\n"
        "3. Monitor for CFO's response in real-time\n"
    ...
    try:
        ...
        console.print("\n" + "="*60)
        console.print("[bold]STEP 1: Generating Emails[/bold]")
        console.print("="*60)
        ...
        console.print("\n" + "="*60)
        console.print("[bold]STEP 2: Submitting for Approval[/bold]")
        console.print("="*60)
        ...
        console.print(Panel.fit(
            ...
            "[dim]Starting monitoring in 3 seconds...[/dim]",
            border_style="green"
        ))
        ...
        console.print("\n" + "="*60)
        console.print("[bold]STEP 3: Monitoring for Approvals[/bold]")
        console.print("="*60)
        monitor_approvals(settings, check_interval=check_interval)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Automated Email Generation and Approval Workflow",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python automation.py                # Run with default 1-second interval
  python automation.py --interval 5   # Run with 5-second check interval
  python automation.py --interval 10  # Run with 10-second check interval
"""
    )
    
    parser.add_argument('--interval', type=int, default=1,
                       help='Check interval in seconds for monitoring approvals (default: 1)')
    
    args = parser.parse_args()
    
    # Run the automation
    run_automation(check_interval=args.interval)
